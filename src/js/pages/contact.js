...
<button-text onclick="groups.delete()" id="groupsDelete" style="display:none;margin-top:2em;" label="confirmDelete"></button-text>`;
	...
	static sendRequestForFriendship(id) {
		...
		communication.ajax({
			url: global.serverApi + 'db/one',
			webCall: 'contact.sendRequestForFriendship',
			method: 'POST',
	...
	static toggleBlockUser(id) {
		...
		if (!e.getAttribute('blockID')) {
			communication.ajax({
				...
				webCall: 'contact.toggleBlockUser',
				success(r) {
					if (r) {
						var v = JSON.parse(r);
	...
	static toggleLocation(id) {
		...
		if (!e.innerHTML) {
			lists.load({
				webCall: 'contact.toggleLocation',
...
class groups {
	...
	static loadListGroups() {
		...
		if (!v)
			return;
		lists.load({
			webCall: 'contact.loadListGroups',
			latitude: geoData.getCurrent().lat,
			longitude: geoData.getCurrent().lon,
