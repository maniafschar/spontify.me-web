...
class intro {
	...
	static openHint(data) {
		...
		var e = ui.q('main:last-child hint'), body = (data.desc.indexOf(' ') > -1 ? data.desc : ui.l('intro.' + data.desc))
			+ (user.contact || intro.currentStep > -1 || location.pathname.length > 1 && location.pathname.indexOf('index.html') < 0 ? '' : '<br/><br/><button-text onclick="ui.navigation.goTo(&quot;login&quot;)" label="login.action"></button-text>')
			+ (data.hinky ? '<hinky style="' + data.hinky + '" class="' + data.hinkyClass + '"></hinky>' : '')
			+ '<close onclick="intro.close()">x</close>';
		body = body.replace('<rating/>', '<br/><br/><input-rating ui="rating"></input-rating><br/><br/><input type="email" name="email" placeholder="Email"></input><br/><br/><textarea name="feedback" maxlength="1000"></textarea><br/><br/><button-text onclick="intro.save()" name="feedback" label="✓"></button-text>');
		body = body.replace('<language/>', '<br/><br/><button-text ' + (global.language == 'DE' ? 'class="favorite"' : '') + ' onclick="intro.language(&quot;DE&quot;)" l="DE" label="Deutsch"></button-text><button-text class="' + (global.language == 'EN' ? ' favorite' : '') + '" onclick="intro.language(&quot;EN&quot;)" l="EN" label="English"></button-text>');
		if (e != ui.q('hint'))
